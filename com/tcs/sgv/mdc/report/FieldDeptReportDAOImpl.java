...
public class FieldDeptReportDAOImpl extends GenericDaoHibernateImpl {
	
	
	
	private SessionFactory sessionFactory = null;
	Session ghibSession = null;
	public FieldDeptReportDAOImpl(Class type, SessionFactory sessionFactory) {
		super(type);
	...
	public List getField_ID(String Loc_code) {
		...
		try {
			...
			lSBQuery = new StringBuffer(" SELECT loc_Id,loc_Name FROM Cmn_Location_Mst  where parent_Loc_Id =:loccode ");
	...
	public Long getFieldCount( String lStrAdminName) {
...
public Long getEnteredFieldCount(String lStrAdminName) {
...
public List getTotalDDOCount(String fie) {
		...
		try {
			...
			lSBQuery = new StringBuffer(" SELECT admin.LOC_ID,FIE.LOC_NAME,COUNT( DISTINCT DDO.DDO_CODE),Fie.LOC_ID,Count(EMP.DCPS_EMP_ID),ADMIN.LOC_NAME FROM ORG_DDO_MST DDO ");
...
public List getTotalDDOCountEnter(String Field) {
...
public String getAdminID( String admin_id) {
...
/*
public List getDDO_Field_ID(String admin_id) {
		
		List lLstResult11 = null;
		//Map fieldMap=new HashMap();
		StringBuffer lSBQuery = null;
		...
		try {		
			...
			lSBQuery = new StringBuffer(" SELECT COUNT(DDO.DDO_CODE), Fie.LOC_ID,FIE.LOC_NAME FROM ORG_DDO_MST DDO ");
			lSBQuery.append("  		 INNER JOIN CMN_LOCATION_MST FIE ON FIE.LOC_ID = DDO.HOD_LOC_CODE   ");
			lSBQuery.append("   INNER JOIN CMN_LOCATION_MST FIE ON FIE.LOC_ID = DDO.HOD_LOC_CODE    ");
			lSBQuery.append("  WHERE DDO.ACTIVATE_FLAG = 1 and admin.LOC_ID=10001 ");
			lSBQuery.append(" GROUP BY  FIE.LOC_NAME,FIE.LOC_ID admin.LOC_ID =:fiecode1   ");
			
		Query lQuery = ghibSession.createSQLQuery(lSBQuery.toString());
		//lQuery.setParameter("loccode", Loc_code);
		lQuery.setParameter("fiecode1", admin_id);
		logger.info("==> getGpfForClassFourTotalDtls query :: "+lQuery);
		logger.info("==> getGpfForClassFourTotalDtls query :: "+lSBQuery.toString());
		  lLstResult11 = lQuery.list();
				
		  logger.info(" Total List size of DDo count entered  ******.. "+lLstResult11.size());	   	    	 			 			 		
...
public List getSumOfDDOCount(String fie) {
...
public List getSumOfDDOCountEnter(String Field) {
