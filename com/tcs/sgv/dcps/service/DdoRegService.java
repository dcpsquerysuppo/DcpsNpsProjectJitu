...
public class DdoRegService extends ServiceImpl 
	 {
...
public List getFinyear() {
	...
	public ResultObject DDORegistrationReport(Map inputMap) {
		...
		/*	DummyOfficeDAO lObjDummyOfficeDAO = new DummyOfficeDAOImpl(
			...
			DdoRegDao lObjDdoRegDAO = new DdoRegDAOImpl(null,serv.getSessionFactory()); 
			
			  Map loginDetailsMap = (Map) inputMap.get("baseLoginMap");
			
				  
			  
			  List lstGenralReport=lObjDdoRegDAO.getDdoGeneralReport();
	            //      lstAisType  = lObjAlIndSer.getAISlist();
	           
	           // List treasury=lObjOldTransactionIdUpdateDAO.getAllTreasuries();

	             inputMap.put("lstGenralReport",lstGenralReport);  
	          //  inputMap.put("treasury",treasury);   
/*public List getFinyear() {
...
*/
   		...
   		// gLogger.info("allowedOrNot for paybill " + allowedOrNot);
			...
			/* Get Years */
	        	//List lLstYears = lObjDcpsCommonDAO.getFinyears();
		//	List lLstYears = lObjDdoRegDAO .getFinyears();
	//	List lLstMonths= lObjDcpsCommonDAO.getMonths();
		/*	List lLstDummyOffice= lObjDummyOfficeDAO.getDummyOffices();*/
		...
		//		inputMap.put("lLstYears", lLstYears);
			...
			resObj.setResultValue(inputMap);
			//resObj.setViewName("DdoRegReport");
			resObj.setViewName("DdoRegReport");
	...
	public ResultObject RegisterDDO(Map inputMap) {
		...
		/*	DummyOfficeDAO lObjDummyOfficeDAO = new DummyOfficeDAOImpl(
			...
			DdoRegDao lObjDdoRegDAO = new DdoRegDAOImpl(null,serv.getSessionFactory()); 
			  ...
			  if(!StringUtility.getParameter("ddo_reg_no",request).equalsIgnoreCase("") && StringUtility.getParameter("ddo_reg_no",request) != null
					  && !StringUtility.getParameter("flag",request).equalsIgnoreCase("") && StringUtility.getParameter("flag",request) != null
					  && !StringUtility.getParameter("ddocode",request).equalsIgnoreCase("") && StringUtility.getParameter("ddocode",request) != null){
				  String ddo_reg_no=StringUtility.getParameter("ddo_reg_no",request);
				  String ddocode=StringUtility.getParameter("ddocode",request);
				 System.out.print("here **********");
				 lObjDdoRegDAO.registerDDO(ddo_reg_no,ddocode);
	...
	/*		  
			  if(!StringUtility.getParameter("ddo_reg_no",request).equalsIgnoreCase("") && StringUtility.getParameter("ddo_reg_no",request) != null){
				  String ddo_reg_no=StringUtility.getParameter("ddo_reg_no",request);
				  int count=lObjDdoRegDAO.isDdoRegExist(ddo_reg_no);
				...
				String cnt=Integer.toString(count);
				  StringBuffer returnMsg = new StringBuffer();
					...
					returnMsg.append("<MESSAGE>");
					
					returnMsg.append(cnt);
				...
				//	logger.info("count "+count);
					...
					inputMap.put("ajaxKey", stateNameIdStr);
					resObj.setResultValue(inputMap);
			  ...
			  if(!StringUtility.getParameter("ddoCode",request).equalsIgnoreCase("") && StringUtility.getParameter("ddoCode",request) != null){
				  String ddoCode=StringUtility.getParameter("ddoCode",request);
				  List lstDdoDetailForReg=lObjDdoRegDAO.getDdoDetailForReg(ddoCode);
				 
				
				  //String ddocode="";
				  String ddoReg1="";
				 if(lstDdoDetailForReg.size()>0){
				  Object ddoReg[] = (Object[]) lstDdoDetailForReg.get(0);
				   ddoCode=ddoReg[4].toString().trim();
				  ddoReg1=ddoReg[5].toString().trim();
				 ...
				 inputMap.put("ddoReg",ddoReg1);  
				  inputMap.put("ddocode",ddoCode);  
		             inputMap.put("lstDdoDetailForReg",lstDdoDetailForReg);  
				
		       	 
				 
			  }
			  resObj.setResultValue(inputMap);
			  resObj.setViewName("DdoRegistration");
	...
	public ResultObject CheckDdoExist(Map inputMap) {
		...
		/*	DummyOfficeDAO lObjDummyOfficeDAO = new DummyOfficeDAOImpl(
			...
			DdoRegDao lObjDdoRegDAO = new DdoRegDAOImpl(null,serv.getSessionFactory()); 
			  ...
			  if(!StringUtility.getParameter("ddo_reg_no",request).equalsIgnoreCase("") && StringUtility.getParameter("ddo_reg_no",request) != null){
				  String ddo_reg_no=StringUtility.getParameter("ddo_reg_no",request);
				  List count=lObjDdoRegDAO.isDdoRegExist(ddo_reg_no);
				 
				  String ddoCodes=null;
				  String ddoOffice=null;
				  String ddoRegs=null;
				  
				  if(count.size()>0){
					  Object ddoReg[] = (Object[]) count.get(0);
					   ddoCodes=ddoReg[0].toString().trim();
					   ddoOffice=ddoReg[1].toString().trim();
					  ddoRegs=ddoReg[2].toString().trim();
				  }
				...
				/*	inputMap.put("cnt", cnt);*/
				 ...
				 String sizes=Integer.toString(size);
					returnMsg.append("<XMLDOC>");
					returnMsg.append("<SIZE>");
					
					returnMsg.append(sizes);
					
					returnMsg.append("</SIZE>");
					returnMsg.append("<DDO>");
					
					returnMsg.append(ddoCodes);
					
					returnMsg.append("</DDO>");
					
					returnMsg.append("<OFFICE>");
					
					returnMsg.append(ddoOffice);
					
					returnMsg.append("</OFFICE>");
					returnMsg.append("<REG>");
					
					returnMsg.append(ddoRegs);
