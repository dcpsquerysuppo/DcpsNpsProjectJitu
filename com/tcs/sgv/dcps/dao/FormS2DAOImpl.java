...
public class FormS2DAOImpl extends GenericDaoHibernateImpl implements FormS2DAO {
    private final Log gLogger;
    org.hibernate.Session ghibSession;

    public FormS2DAOImpl(Class type, SessionFactory sessionFactory) {
        super(type);
    ...
    public List getSectionADetailsNew(String lStrSevaarthId) throws Exception {
        ...
        Date lDtCurrDate = SessionHelper.getCurDate();
        try {
            lStrQuery.append(" SELECT upper(emp.emp_name),emp.gender,to_char(emp.dob,'ddMMyyyy'),upper(emp.FATHER_OR_HUSBAND),upper(nvl(emp.ADDRESS_BUILDING,' ')) as block_no, substr(upper(nvl(emp.ADDRESS_STREET,' ')||' '||nvl(emp.LANDMARK,' ')),1,30) as premise, upper(nvl(emp.LOCALITY,' ')) as area, ");
            lStrQuery.append(" upper(nvl(emp.DISTRICT,' ')),upper(nvl(st.STATE_NAME,' ')),cast('INDIA' as varchar) as country ,upper(nvl(cast (emp.PINCODE as varchar),' ')),substr(upper(nvl(emp.CNTCT_NO,' ')),4,11),upper(nvl(emp.CELL_NO,' ')),  ");
            lStrQuery.append(" nvl(emp.EMAIL_ID,' '), nvl(emp.bank_acnt_no,''),upper(nvl(bank.BANK_NAME,'')),upper(nvl(branch.BRANCH_NAME,'')),  nvl(upper(branch.NEW_ADDRESS),' '), emp.IFSC_CODE,branch.MICR_CODE,emp.salutation,  nvl(emp.PAN_NO,' '), upper(nvl(emp.PADDRESS_BUILDING,' ')), substr(upper(nvl(emp.PADDRESS_STREET,' ')||' '||nvl(emp.PLANDMARK,' ')),1,30), ");
            lStrQuery.append(" upper(nvl(emp.PLOCALITY,' ')), upper(nvl(emp.PDISTRICT,' ')),upper(nvl(st.STATE_NAME,' ')),cast('INDIA' as varchar) as country , upper(nvl(cast (emp.PPINCODE as varchar),' ')),upper(nvl(cast (branch.PINCODE as varchar),' ')),  ");
            lStrQuery.append(" nvl(emp.PRAN_NO,' '),org.EMP_FNAME,org.EMP_MNAME,org.EMP_LNAME,org.EMP_FNAME||' '||org.EMP_MNAME||' '||org.EMP_LNAME,substr(upper(nvl(emp.CNTCT_NO,' ')),1,3) as std_code,substr(PAN_NO,1,10) FROM  mst_dcps_emp emp  ");
            lStrQuery.append(" inner join org_emp_mst org on org.EMP_Id = emp.ORG_EMP_MST_ID  ");
            lStrQuery.append(" inner join CMN_STATE_MST st on st.STATE_ID = emp.STATE  ");
    ...
    public List getSectionCDetailsNew(String lStrSevaarthId) throws Exception {
        ...
        Date lDtCurrDate = SessionHelper.getCurDate();
        try {
            lStrQuery.append(" SELECT upper(decode(form.DCPS_NMN_NAME,'NA',' ','',' ',form.DCPS_NMN_NAME)),nvl(to_char(form.DCPS_NMN_DOB,'ddMMyyyy'),' '), UPPER(case when form.DCPS_NMN_RLT = '-1' then ' ' when look1.LOOKUP_NAME  is null then form.DCPS_NMN_RLT  else look1.LOOKUP_NAME end), ");
            lStrQuery.append(" (case when form.DCPS_NMN_SHARE  is null then ' ' else form.DCPS_NMN_SHARE||'%' end),upper(decode(form.DCPS_NMN_GUARDIAN,'NA',' ','',' ',form.DCPS_NMN_GUARDIAN))  ,UPPER(nvl(form.DCPS_NMN_INVALID,' ')) ");
            ...
            lStrQuery.append(" WHERE  emp.REG_STATUS=1  AND (emp.EMP_SERVEND_DT >= sysdate OR emp.EMP_SERVEND_DT is null)  and emp.FORM_STATUS=1 and emp.AC_DCPS_MAINTAINED_BY in (700174,700240,700241,700242)  ");
            if (!"".equals(lStrSevaarthId) && lStrSevaarthId != null) {
                lStrQuery.append(" AND UPPER(emp.SEVARTH_ID) = :lStrSevaarthId");
            }
            hqlQuery = session.createSQLQuery(lStrQuery.toString());
    ...
    public List getEmpListForFrmS1Edit(String strDDOCode, String flag, String txtSearch, String isDeputation) {
        ...
        strQuery.append(" desig.DSGN_NAME , ");
    	if(isDeputation.equals("Y"))
		{
		...
		{
			strQuery.append(" mstddo.DDO_CODE, ");
		}
        		
    	strQuery.append(" empmst.DCPS_ID ");
     ...
     /*   if(isDeputation.equals("Y"))
        ...
        strQuery.append(" and empmst.DCPS_OR_GPF='Y' and empmst.reg_status=1 and empmst.AC_DCPS_MAINTAINED_BY in (700174,700240,700241,700242) and userpost.ACTIVATE_FLAG = 1 and post.ACTIVATE_FLAG = 1 ");
