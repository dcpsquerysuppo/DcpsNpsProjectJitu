...
public class Form2ReportPayArrear extends DefaultReportDataFinder implements
ReportDataFinder {
	...
	throws ReportException {
		...
		try {
			...
			if (report.getReportCode().equals("700053")) {
				...
				lSBQuery
				.append(" SELECT EM.dcps_id,EM.emp_name,TR.startDate,TR.endDate,TR.BASIC_PAY,TR.DP,TR.DA,TR.contribution,TR.ddo_code,HPP.CDA,HPP.DED_ADJUST ");
				lSBQuery
				.append(" FROM trn_dcps_contribution TR,mst_dcps_emp EM");

				// Added later
				lSBQuery.append(" ,hr_eis_emp_mst HE,HR_PAY_PAYBILL HPP,PAYBILL_HEAD_MPG PHM ,CMN_LOOKUP_MST look");
				// Added later

				lSBQuery.append(" WHERE TR.dcps_emp_id = EM.dcps_emp_id ");
				lSBQuery.append("  and	em.AC_DCPS_MAINTAINED_BY = look.LOOKUP_ID	");
				...
				lSBQuery.append(" AND TR.TYPE_OF_PAYMENT = 700049");
				...
				if (lLstFinal != null && !lLstFinal.isEmpty()) {
					...
					while (it.hasNext()) {
						...
						{
							td.add(Round(Double
									.parseDouble(tuple[4].toString()), 0));
						} else {
							td.add("");
						}
						...
						{
							td.add(Round(Double
									.parseDouble(tuple[5].toString()), 0));
						} else {
							td.add("");
						}
						...
						if( Long.parseLong(tuple[9].toString())==0)
						{
							...
							{
								td.add(Round(Double
										.parseDouble(tuple[6].toString()), 0));
							} else {
								td.add("");
							}
						...
						{
							td.add(Round(Double
									.parseDouble(tuple[7].toString()), 0));
							lDoubleTotalContribution = Double
							.parseDouble(tuple[7].toString())
				...
				{
					RptVo = reportsDao.getReport("700054", report.getLangId(),
				...
				RptVoForHiddenColumns = reportsDao.getReport("700054", report
				...
				if(!lStrAccMaintainedBy.equals(""))
				{
					...
					else
					{
						...
						lStrThirdLastLineInTable.append
						("Government Employee's Contribution Tier-1(8342 508-101),32-Contributions");
						...
						("Head of Account 8342-Other Deposits,117-Government Employees Defined Contribution Pension scheme (02)(02)- Defined Contribution Pension Scheme,Government Employer's Contribution Tier-1(8342 509-901),32-Contributions");
	...
	public List getBillGroup(Hashtable otherArgs, String lStrLangId,
		...
		try {
			...
			sb.append("SELECT dcpsDdoBillGroupId, dcpsDdoBillDescription FROM MstDcpsBillGroup WHERE billDeleted is null ");
