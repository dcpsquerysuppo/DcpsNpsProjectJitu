...
public class TranIdGenBillNotGenReport extends DefaultReportDataFinder {
    ...
    throws ReportException {
        ...
        try {
            ...
            if (report.getReportCode().equals("9000240")) {
...
inner join mst_dto_reg dto on dto.loc_id=loc.loc_id
           ...
           // --    inner join PAYBILL_HEAD_MPG head on head.LOC_ID=ddo.LOCATION_CODE 
            // --   inner join HR_PAY_PAYBILL paybill on paybill.PAYBILL_GRP_ID=head.PAYBILL_ID 
            	...
            	SBQueryQuery.append(" SELECT loc.loc_id,loc.loc_name,dto.dto_reg_no,count(case when bh.file_status=0 then 1 else null end) as  Gen_file_count,   ");
            	SBQueryQuery.append(" count( case when bh.file_status=1 and bh.transaction_id is null then 1 else null end) as val_file_count,  ");
            	SBQueryQuery.append(" count( case when bh.TRANSACTION_ID is not null and (nsdl.BILL_STATUS=-1 or nsdl.BILL_STATUS is null) then 1 else null end) as  tran_id_gen_bill_not_gen,   ");
            	SBQueryQuery.append(" count( case when bh.TRANSACTION_ID is not null and nsdl.BILL_STATUS in (0,1,3) and bh.file_status in (5,11) and nsdl.BILL_STATUS is not null then 1 else null end) as  tran_mtr45_file_count,  "); 
            	SBQueryQuery.append(" count( case when nsdl.BILL_STATUS=1 then 1 else null end) as approve_bill,   ");
                  
            	SBQueryQuery.append(" count(case when bh.file_status=0 then 1 else null end) +   ");
            	SBQueryQuery.append(" count( case when bh.file_status=1 and bh.transaction_id is null then 1 else null end) +   ");
            	SBQueryQuery.append(" count( case when bh.TRANSACTION_ID is not null and (nsdl.BILL_STATUS=-1 or nsdl.BILL_STATUS is null) then 1 else null end) +   ");
            	SBQueryQuery.append("  count( case when bh.TRANSACTION_ID is not null and nsdl.BILL_STATUS in (0,1,3) and bh.file_status in (5,11) then 1 else null end) +  ");
            	SBQueryQuery.append(" count( case when nsdl.BILL_STATUS=1 then 1 else null end) as total FROM nsdl_bh_dtls bh   ");
                  
            	SBQueryQuery.append(" left outer join nsdl_bill_dtls nsdl on nsdl.file_name=bh.file_name  ");
            	SBQueryQuery.append(" inner join CMN_LOCATION_MST loc on substr(loc.loc_id,1,2) = substr(bh.file_name,1,2) and loc.DEPARTMENT_ID=100003   ");
            	SBQueryQuery.append(" inner join mst_dto_reg dto on dto.loc_id = loc.loc_id   ");
                 //-- inner join ORG_DDO_MST ddo on substr(ddo.DDO_CODE,1,4)=substr(loc.LOC_ID,1,4)  
            	SBQueryQuery.append(" where bh.status <> -1  ");
             	//-- and ddo.DDO_CODE <> '1111222222'  
            	SBQueryQuery.append(" and bh.month="+month+" and bh.year="+year+"  ");
            	SBQueryQuery.append(" group by loc.loc_name,loc.loc_id,dto.dto_reg_no ");
            	SBQueryQuery.append(" order by loc.loc_id ");
