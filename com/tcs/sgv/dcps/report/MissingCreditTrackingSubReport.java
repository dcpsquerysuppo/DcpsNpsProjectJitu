...
public class MissingCreditTrackingSubReport extends DefaultReportDataFinder {
    ...
    throws ReportException {
        ...
        try {
            ...
            if (report.getReportCode().equals("9000345")) {
...
/*
				...
				gLogger.info("#################sub report year id"+lStrYearId);
				gLogger.info("#################sub report month id"+lStrMonthId);
				gLogger.info("#################sub reprot treasuryCode"+treasuryCode);
               // String treasuryCode = ((String) report.getParameterValue("treasuryCode"));
...
inner join mst_dto_reg dto on dto.loc_id=loc.loc_id
           ...
           // --    inner join PAYBILL_HEAD_MPG head on head.LOC_ID=ddo.LOCATION_CODE 
            // --   inner join HR_PAY_PAYBILL paybill on paybill.PAYBILL_GRP_ID=head.PAYBILL_ID 
              ...
              if(locationId==10702){
            	  	...
            	  	SBQueryQuery.append(" SELECT emp.emp_name,emp.dcps_id||'/'||emp.pran_no,loc.loc_name,trn.ddo_code, trn.scheme_code,trn.month_id,fin.fin_year_desc,trn.contribution,look.lookup_name,'V No.: '||trn.voucher_no||' - V Date: '||to_char(trn.voucher_date,'dd/MM/yyyy'),to_char(trn.CREATED_DATE,'dd/MM/yyyy') FROM trn_dcps_contribution trn  ");
                	SBQueryQuery.append(" inner join cmn_location_mst loc on substr(loc.loc_id,1,2)=substr(trn.treasury_code,1,2)  ");
                	SBQueryQuery.append(" inner join mst_dcps_emp emp on emp.dcps_emp_id=trn.dcps_emp_id ");
                	SBQueryQuery.append(" inner join cmn_lookup_mst look on look.lookup_id=trn.type_of_payment ");
                	SBQueryQuery.append(" inner join sgvc_fin_year_mst fin on fin.fin_year_id=trn.fin_year_id ");
                	SBQueryQuery.append(" where trn.IS_MISSING_CREDIT='Y' and to_char(trn.created_date,'MM')="+lStrMonthId+" and to_char(trn.created_date,'YYYY')="+lStrYearId+" and loc.loc_id='"+treasuryCode+"' ");
                	SBQueryQuery.append(" order by loc.loc_id ");
           	  	...
           	  	SBQueryQuery.append(" SELECT emp.emp_name,emp.dcps_id||'/'||emp.pran_no,loc.loc_name,trn.ddo_code, trn.scheme_code,trn.month_id,fin.fin_year_desc,trn.contribution,look.lookup_name,'V No.: '||trn.voucher_no||' - V Date: '||to_char(trn.voucher_date,'dd/MM/yyyy'),to_char(trn.CREATED_DATE,'dd/MM/yyyy') FROM trn_dcps_contribution trn  ");
               	SBQueryQuery.append(" inner join cmn_location_mst loc on loc.loc_id=trn.treasury_code ");
               	SBQueryQuery.append(" inner join mst_dcps_emp emp on emp.dcps_emp_id=trn.dcps_emp_id ");
               	SBQueryQuery.append(" inner join cmn_lookup_mst look on look.lookup_id=trn.type_of_payment ");
               	SBQueryQuery.append(" inner join sgvc_fin_year_mst fin on fin.fin_year_id=trn.fin_year_id ");
               	SBQueryQuery.append(" where trn.IS_MISSING_CREDIT='Y' and to_char(trn.created_date,'MM')="+lStrMonthId+" and to_char(trn.created_date,'YYYY')="+lStrYearId+" and trn.treasury_code='"+treasuryCode+"' ");
               	SBQueryQuery.append(" order by loc.loc_id  ");
        ...
        //  String urlPrefix1 = "ifms.htm?actionFlag=reportService&reportCode=9000326&action=generateReport";
                StrSqlQuery = SBQueryQuery.toString();
                System.out.print("#############sub report Query is : "+StrSqlQuery);
