...
public class NsdlDdoWiseReport extends DefaultReportDataFinder {
    ...
    throws ReportException {
        ...
        try {
            ...
            if (report.getReportCode().equals("9000180")) {
...
*/
            	...
            	SBQueryQuery.append(" SELECT ddo.DDO_CODE,reg.ddo_reg_no,count(distinct hreis.emp_id),sum(paybill.DED_ADJUST)  as employye,   ");//sum(paybill.DCPS)+sum(paybill.DCPS_PAY)+sum(paybill.DCPS_DELAY)+sum(paybill.DCPS_DA) as employye
        		SBQueryQuery.append(" sum(paybill.DED_ADJUST),count(distinct head.PAYBILL_ID),sum(paybill.NET_TOTAL),sum(paybill.GROSS_AMT) FROM PAYBILL_HEAD_MPG head   inner join NSDL_PAYBILL_DATA paybill on paybill.PAYBILL_GRP_ID =head.PAYBILL_ID   ");
        		...
        		//SBQueryQuery.append("  left outer join NSDL_BH_DTLS nsdl on substr(nsdl.file_name,1,4)=loc.LOC_ID and nsdl.TRANSACTION_ID is null and nsdl.year= "+year+" and nsdl.MONTH= "+month+"  ");
        		
        		SBQueryQuery.append(" where head.PAYBILL_YEAR="+year+" and head.PAYBILL_MONTH="+month+"  and substr(ddo.ddo_code,1,4)='"+locID+"' and head.APPROVE_FLAG=1 ");
        		if(pranWise.equals("true1")){
            		
            		SBQueryQuery.append(" and mstemp.PRAN_NO not in (SELECT sd.SD_PRAN_NO FROM NSDL_SD_DTLS sd inner join NSDL_BH_DTLS bh on bh.FILE_NAME=sd.FILE_NAME and bh.TRANSACTION_ID is not null and bh.STATUS <>-1 and bh.YEAR = "+year+" and bh.MONTH="+month+" and bh.file_name like '"+locID+"%')");
            		
            		
            		SBQueryQuery.append(" and mstemp.PRAN_NO is not null ");
            		}else{
            			SBQueryQuery.append(" and ((mstemp.PRAN_NO not in (SELECT sd.SD_PRAN_NO FROM NSDL_SD_DTLS sd inner join NSDL_BH_DTLS bh on bh.FILE_NAME=sd.FILE_NAME and bh.TRANSACTION_ID is not null and  bh.STATUS <>-1 and bh.YEAR = "+year+" and bh.MONTH="+month+" and bh.file_name like '"+locID+"%')) or (mstemp.PRAN_NO is null))");
            		}
        		
        		
        	//	SBQueryQuery.append(" and mstemp.DCPS_OR_GPF='Y'  and mstemp.AC_DCPS_MAINTAINED_BY=700174 ");
           		SBQueryQuery.append("  group by ddo.DDO_CODE,reg.ddo_reg_no having sum(paybill.DCPS)+sum(paybill.DCPS_PAY)+sum(paybill.DCPS_DELAY)+sum(paybill.DCPS_DA) >0  order by reg.ddo_reg_no ");
            	...
            	String urlPrefix1 = "ifms.htm?actionFlag=reportService&reportCode=9000207&action=generateReport&DirectReport=TRUE&displayOK=TRUE";
               ...
               String amount=null;
                while (rs.next()) {
                  ...
                  //  rowList.add(new StyledData(rs.getString(1), CenterAlignVO));
                    ...
                    rowList.add(new URLData(rs.getString(8), urlPrefix1+ "&treasuryCode="+rs.getString(1)
            				+"&year="+year+"&month="+month+"&pranWise="+pranWise
			));
