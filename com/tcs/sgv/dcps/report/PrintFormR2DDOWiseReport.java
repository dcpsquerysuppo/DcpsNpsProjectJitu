...
public class PrintFormR2DDOWiseReport extends DefaultReportDataFinder {
	...
	try {
		...
		if (report.getReportCode().equals("700039")) {
			...
			lSBQuery.append(" SELECT EM.emp_name,EM.dcps_id,TR.scheme_code,VC.voucher_no,VC.voucher_date,TR.contribution,TR.EMPLOYER_CONTRI_FLAG");
			...
			lSBQuery.append(" AND TR.ddo_code = :ddoCode");
			...
			if (lLstFinal != null && !lLstFinal.isEmpty())
			{
				...
				while (it.hasNext())
				{
					...
					if(tuple[5] != null && tuple[6] != null ) // Employer's contribution. Both are same as of now.
					{
						if(tuple[6].toString().equals("Y"))
						{
		        ...
		        String header4 = "<b>" + "<font size=\"1px\"> " + "DDO : " + lStrDDOName + "</font></b></br>" ;
		        String header5 = "<b>" + "<font size=\"1px\"> " + "Name of Treasury / Sub-Treasury : " + lStrTreasuryName + " / " + gStrLocCode + space(5) + "Date :- " + space(2)  + curDate + "</font>" + "</b>" + "</p>"  ;			
					        
				String additionalHeader =  header1 + header2 + header3 + header4 + header5 + "<br>" ;
	...
	public List getYear(String lStrLangId, String lStrLocId) {
	 ...
	 public List getAllDDOsInTreasury(Hashtable otherArgs,String lStrLangId, String lStrLocCode) 
	 {
			...
			sb.append("WHERE RO.locationCode = :locationCode AND RO.ddoCode = DM.ddoCode AND LM.locationCode = RO.locationCode");
