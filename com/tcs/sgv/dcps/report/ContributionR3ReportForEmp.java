			...
			if (report.getReportCode().equals("700072")) {
				...
				if (rs.next()) {
					...
					for (int i = 4; i <= 12; i++) {
						...
						SBQuery.append("SELECT contribution,EMPLOYER_CONTRI_FLAG FROM trn_dcps_contribution WHERE dcps_emp_id = " + empId + " AND REG_STATUS = 1 AND STATUS = 'H' AND FIN_YEAR_ID = " + yearId + " AND month_id = " + i);
