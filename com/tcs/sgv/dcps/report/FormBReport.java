...
public class FormBReport extends DefaultReportDataFinder implements ReportDataFinder 
{
	private static final Logger gLogger = Logger.getLogger(FormBReport.class);
	...
	private ResourceBundle gObjRsrcBndle = ResourceBundle.getBundle("resources/dcps/terminalFormB");
	...
	public Collection findReportData(ReportVO report, Object criteria) throws ReportException 
	{
		...
		if(report.getReportCode().equals("80000854"))
		{
			...
			{			
				...
				reasonOfTerm = tuple[16].toString();
			...
			rowList.add(new StyledData(gObjRsrcBndle.getString("FORM.FormB")+newline+newline,centerAlign));
			...
			rowList = new ArrayList<Object>();
			rowList.add(new StyledData(ddoOffice+newline,rightAlign));
			dataList.add(rowList);

			rowList = new ArrayList<Object>();
			rowList.add(new StyledData("Date : "+dateFormat5.format(today)+newline,rightAlign));
			dataList.add(rowList);
			...
			rowList = new ArrayList<Object>();
			rowList.add(new StyledData(treasuryName,leftAlign));
			dataList.add(rowList);
			...
			rowList = new ArrayList<Object>();
			rowList.add(new StyledData(gObjRsrcBndle.getString("FORM.SALUTATION4")+newline+newline,leftAlign));
			dataList.add(rowList);
			...
			rowList.add(new StyledData(gObjRsrcBndle.getString("FORM.FORM1")+"__"+empName+"__"+gObjRsrcBndle.getString("FORM.FORM2")+"__"+doj+"__"
					...
					gObjRsrcBndle.getString("FORM.FORM5")+"__"+reasonOfTerm+"__"+gObjRsrcBndle.getString("FORM.FORM6")+"__"+reasonOfTerm+"__"
					+gObjRsrcBndle.getString("FORM.FORM7")+"__"+dateOfTermination+"__"+gObjRsrcBndle.getString("FORM.FORM8")+newline
					,leftAlign));
			dataList.add(rowList);
			...
			{			
				for (int i=0; i<lLstEmpContriDetails.size();i++)
				{
					...
					td1.add(designation);
					formS1Id= tuple[6].toString();	
					address1 = tuple[10].toString();
		...
		if(report.getReportCode().equals("80000855"))
		{
			report.setStyleList(rowsFontsVO);
			lStrSevaarthId = ((String) report.getParameterValue("sevarthId")).trim();
			gLogger.info("lStrSevaarthId*************" + lStrSevaarthId);
...
//				voucherDate = tuple2[4].toString();
			...
			rowList = new ArrayList<Object>();
			rowList.add(new StyledData(gObjRsrcBndle.getString("FORM.FORM9")+"__"+lastMonth+"__"+gObjRsrcBndle.getString("FORM.FORM10")+"__"+lastYear+"__"
					+gObjRsrcBndle.getString("FORM.FORM11")+"__"+voucherNo+"__"+gObjRsrcBndle.getString("FORM.FORM12")+"__"+voucherDate+"__"+gObjRsrcBndle.getString("FORM.FORM13")+newline
					,leftAlign));
			dataList.add(rowList);

			rowList = new ArrayList<Object>();
			rowList.add(new StyledData(gObjRsrcBndle.getString("FORM.FORM14")+newline+newline,leftAlign));
			dataList.add(rowList);

			rowList = new ArrayList<Object>();
			rowList.add(new StyledData(gObjRsrcBndle.getString("FORM.FORM15")+space(200)+gObjRsrcBndle.getString("FORM.FORM18"),leftAlign));
			dataList.add(rowList);

			rowList = new ArrayList<Object>();
			rowList.add(new StyledData(gObjRsrcBndle.getString("FORM.FORM16")+space(238)+gObjRsrcBndle.getString("FORM.FORM19"),leftAlign));
			dataList.add(rowList);

			rowList = new ArrayList<Object>();
			rowList.add(new StyledData(gObjRsrcBndle.getString("FORM.FORM17"),leftAlign));
			dataList.add(rowList);

			rowList = new ArrayList<Object>();
			rowList.add(new StyledData(ddoOffice,rightAlign));
			dataList.add(rowList);

			rowList = new ArrayList<Object>();
			rowList.add(new StyledData(gObjRsrcBndle.getString("FORM.FORM20")+space(20),rightAlign));
			dataList.add(rowList);
	...
	public List getContributionDetails(String sevarthID) 
	{
		...
		try
		{
			...
			sb.append(" dcps.ADDRESS_BUILDING ||','|| dcps.ADDRESS_STREET ||','|| dcps.LANDMARK || ',' || dcps.DISTRICT || ',' || dcps.PINCODE as emp_add,cast(a.fin_year as varchar),nvl(s1.FORM_S1_ID,0),  ");
			sb.append(" s1.PRESENT_ADD_FLAT_UNIT_NO_BLOCK_NO ||','|| s1.PRESENT_ADDRESS_NAME_OF_PREMISE_BUILDING_VILLAGE ||','|| s1.PRESENT_ADDRESS_AREA_LOCALITY_TALUKA || ',' || s1.PRESENT_ADDRESS_DISTRICT_TOWN_CITY || ',' || s1.PRESENT_ADDRESS_STATE_UNION_TERRITORY || ',' || s1.PRESENT_ADDRESS_PIN_CODE as s1_add, ");
			sb.append("  cast((a.closeEmplrBal+a.CloseBal)as  DECIMAL(16,2)) as final_amount ,nvl(d2.DSGN_NAME,'0'),nvl(tern.CUR_ADDRESS,'0') ");
	...
	public List getContributionDetailsNew(String sevarthID)
	{
		...
		try
		{
			...
			SQLQuery selectQuery = null;
			sb.append("SELECT dcps.EMP_NAME,dcps.DCPS_ID,to_char(dcps.DOJ,'dd/MM/yyyy'),d.DSGN_NAME,   ");
			sb.append("dcps.ADDRESS_BUILDING ||','|| dcps.ADDRESS_STREET ||','|| dcps.LANDMARK || ',' || dcps.DISTRICT || ',' || dcps.PINCODE as emp_add,cast(fin.FIN_YEAR_DESC as varchar),nvl(s1.FORM_S1_ID,0),   ");
			sb.append("s1.PRESENT_ADD_FLAT_UNIT_NO_BLOCK_NO ||','|| s1.PRESENT_ADDRESS_NAME_OF_PREMISE_BUILDING_VILLAGE ||','|| s1.PRESENT_ADDRESS_AREA_LOCALITY_TALUKA || ',' || s1.PRESENT_ADDRESS_DISTRICT_TOWN_CITY || ',' || s1.PRESENT_ADDRESS_STATE_UNION_TERRITORY || ',' || s1.PRESENT_ADDRESS_PIN_CODE as s1_add,  yr.CLOSE_NET,nvl(d2.DSGN_NAME,'0'),nvl(tern.CUR_ADDRESS,'0')   ");
			sb.append("from MST_DCPS_EMP dcps   ");
			sb.append(" inner join ORG_DESIGNATION_MST d on d.DSGN_ID=dcps.DESIGNATION   ");
			sb.append("left outer join FRM_FORM_S1_DTLS s1 on s1.SEVARTH_ID=dcps.SEVARTH_ID   ");
