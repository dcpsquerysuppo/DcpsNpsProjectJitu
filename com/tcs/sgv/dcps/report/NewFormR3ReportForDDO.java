...
public class NewFormR3ReportForDDO extends DefaultReportDataFinder implements ReportDataFinder
{
	private static final Logger gLogger = Logger.getLogger(NewFormR3ReportForDDO.class);
	...
	public Collection findReportData(ReportVO lObjReport, Object criteria)throws ReportException
	{
		...
		try{
			...
			if (lObjReport.getReportCode().equals("80000666")) 
			{
				...
				for(int monID=3;monID<=14;monID++)
				{
					...
					for (int i =0; i < lDbMissingCreditTier1.size(); i++) {
						...
						if(newIndex == Integer.parseInt(obj[3].toString())){
							...
							rowList.add(nosci(Double.parseDouble(missingamt)).toString());	
							rowList.add(nosci(Double.parseDouble(missingamt)).toString());
							rowList.add("");
				...
				rowList.add("");
				Long TotalTier2=lLngYearlyContriTier2+lLngOpeningTier2;
				rowList.add(nosci(TotalTier2).toString());
				rowList.add("");
	...
	public List getAllEmpsUnderDDO(String lStrLangId,
			String lStrLocId) {
		...
		try {
			//if (!lStrddoCode.equals("-1")) {
				...
				sb
				.append(" WHERE EM.ddoCode = (SELECT ddoCode FROM OrgDdoMst where postId= :postId) and EM.dcpsOrGpf = 'Y' and regStatus = 1  order by EM.name");
				Query selectQuery = lObjSession.createQuery(sb.toString());
				selectQuery.setParameter("postId", gLngPostId);
