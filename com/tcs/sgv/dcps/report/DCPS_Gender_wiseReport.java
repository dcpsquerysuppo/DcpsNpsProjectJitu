...
public class DCPS_Gender_wiseReport extends DefaultReportDataFinder implements ReportDataFinder {
	
	private static final Logger logger = Logger.getLogger(DCPS_Gender_wiseReport.class);
		...
		try {
			...
			boldStyleVO[0].setStyleValue(IReportConstants.VALUE_FONT_WEIGHT_BOLDER);
			  ...
			  SimpleDateFormat objsimpledatefrm = new SimpleDateFormat("dd/MM/yyyy");
				...
				if (report.getReportCode().equals("700109")|| report.getReportCode().equals("700115")|| report.getReportCode().equals("700116")) {
					logger.info("in report 700109  ***********");
					...
					if(lStrAdminDept.equals("")&& lStrFieldDept.equals(""))
					{
						...
						//lLstDeptWiseDtls = getAdminFieldDeptDtls(null);					
				 ...
				 if (!lStrAdminDept.equals(""))
					{
						...
						//lLstDeptWiseDtls = getAdminFieldDeptDtls(Long.parseLong(lStrAdminDept));	
						strheader = "DCPS-Employee Statistics - Gender-wise " + "As On" + objsimpledatefrm.format(date) + "\n (Field Department Wise)"; 	
				 ...
				 if (!lStrFieldDept.equals(""))
					...
					if (lLstFinal != null && !lLstFinal.isEmpty()) {
						...
						/*	roundperoffemale = Math.round(perofFemale*100)/100f;						
							...
							if (!lStrFieldDept.equals(""))
							{
								...
								dataListForTable.add(new URLData(ddocode,"ifms.htm?actionFlag=reportService&reportCode=700112&action=generateReport&ddocode="+ddocode + "&fieldDept="+lStrFieldDept ));
							}
							
							else if (!lStrAdminDept.equals(""))
							{
								dataListForTable.add(new URLData(fieldname,"ifms.htm?actionFlag=reportService&reportCode=700116&action=generateReport&fieldDept="+fieldid +"&adminDept="+lStrAdminDept ));			
												
							}
							else
							{
								dataListForTable.add(new URLData(adminname,"ifms.htm?actionFlag=reportService&reportCode=700115&action=generateReport&adminDept="+adminid ));
				 ...
				 if (report.getReportCode().equals("700112"))
				 {
					...
					if (lLstFinal != null && !lLstFinal.isEmpty()) {
...

						{
							...
							logger.info("Admin Id is  ***********" + strname);	
							strsevarthid = (lObj[1] != null) ? lObj[1].toString(): "";
							logger.info("Admin Id is  ***********" + strsevarthid);	
							strGender = (lObj[2] != null) ? lObj[2].toString(): "";
							logger.info("Admin Id is  ***********" + strGender);
	...
	public List getEmployeebyDDO(Long lStrDDOcode)
	{
		...
		try{
			...
			lSBQuery.append(" INNER JOIN CMN_LOCATION_MST admin on admin.LOC_ID = ddo.DEPT_LOC_CODE ");
				...
				lSBQuery.append(" where em.FORM_STATUS =1 and em.REG_STATUS in (1,2) and ddo.DDO_CODE <> '1111222222' and em.DCPS_OR_GPF='Y' and  ddo.DDO_CODE=:ddocode ");
...
private List getAdminDeptDtls() {
		...
		lSBQuery.append(" inner join CMN_LOCATION_MST cmn on  ddo.DEPT_LOC_CODE=cmn.LOC_ID  where ddo.ACTIVATE_FLAG = 1  and emp.REG_STATUS in (1) and emp.GENDER='M' and DCPS_OR_GPF='Y' and ddo.DDO_CODE <> '1111222222' and cmn.LOC_ID =ocmn.LOC_ID ) MALE_COUNT  ");
		...
		lSBQuery.append("  inner join CMN_LOCATION_MST cmn on  ddo.DEPT_LOC_CODE=cmn.LOC_ID  where ddo.ACTIVATE_FLAG = 1  and emp.REG_STATUS in (1) and emp.GENDER='F' and DCPS_OR_GPF='Y' and ddo.DDO_CODE <> '1111222222' and cmn.LOC_ID =ocmn.LOC_ID) FEMALE_COUNT, ");
		lSBQuery.append("   (select count(ORG_EMP_MST_ID)  FROM mst_dcps_emp emp inner join ORG_DDO_MST ddo on emp.DDO_CODE = ddo.DDO_CODE ");
		lSBQuery.append("  inner join CMN_LOCATION_MST cmn on  ddo.DEPT_LOC_CODE=cmn.LOC_ID  where ddo.ACTIVATE_FLAG = 1  and emp.REG_STATUS in (1) and emp.GENDER='T' and DCPS_OR_GPF='Y' and ddo.DDO_CODE <> '1111222222' and cmn.LOC_ID =ocmn.LOC_ID) TransGender_COUNT  ");
		lSBQuery.append("  ,(select count(ORG_EMP_MST_ID)  FROM mst_dcps_emp emp inner join ORG_DDO_MST ddo on emp.DDO_CODE = ddo.DDO_CODE ");
		lSBQuery.append("  inner join CMN_LOCATION_MST cmn on  ddo.DEPT_LOC_CODE=cmn.LOC_ID  where ddo.ACTIVATE_FLAG = 1  and emp.REG_STATUS in (1) and DCPS_OR_GPF='Y' and ddo.DDO_CODE <> '1111222222' and cmn.LOC_ID =ocmn.LOC_ID) TOTAL_EMP ");
		lSBQuery.append("  from  CMN_LOCATION_MST ocmn where ocmn.DEPARTMENT_ID = 100001  ");
...
lSBQuery.append(" INNER JOIN CMN_LOCATION_MST admin on admin.LOC_ID = ddo.DEPT_LOC_CODE ");
lSBQuery.append("  INNER JOIN CMN_LOCATION_MST fld on fld.LOC_ID = ddo.HOD_LOC_CODE  ");
lSBQuery.append("  where em.FORM_STATUS =1 and em.REG_STATUS in (1,2) and em.GENDER='M' and em.DCPS_OR_GPF='Y' and  ddo.ACTIVATE_FLAG = 1  and ddo.DDO_CODE <> '1111222222' and admin.LOC_ID=cadmin.LOC_ID and fld.loc_id= cfld.loc_id) as MALE_COUNT, ");
lSBQuery.append(" (SELECT count(em.ORG_EMP_MST_ID) FROM  mst_dcps_emp em ");
lSBQuery.append(" INNER JOIN org_ddo_mst ddo on em.DDO_CODE = ddo.DDO_CODE ");
lSBQuery.append(" INNER JOIN CMN_LOCATION_MST admin on admin.LOC_ID = ddo.DEPT_LOC_CODE  ");
lSBQuery.append(" INNER JOIN CMN_LOCATION_MST fld on fld.LOC_ID = ddo.HOD_LOC_CODE ");
lSBQuery.append("  where em.FORM_STATUS =1 and em.REG_STATUS in (1,2) and em.GENDER='F' and em.DCPS_OR_GPF='Y' and  ddo.ACTIVATE_FLAG = 1  and ddo.DDO_CODE <> '1111222222' and admin.LOC_ID=cadmin.LOC_ID and fld.loc_id= cfld.loc_id) as FEMALE_COUNT, ");
lSBQuery.append(" (SELECT count(em.ORG_EMP_MST_ID) FROM  mst_dcps_emp em  ");
lSBQuery.append(" INNER JOIN org_ddo_mst ddo on em.DDO_CODE = ddo.DDO_CODE ");
lSBQuery.append(" INNER JOIN CMN_LOCATION_MST admin on admin.LOC_ID = ddo.DEPT_LOC_CODE	 ");
lSBQuery.append(" INNER JOIN CMN_LOCATION_MST fld on fld.LOC_ID = ddo.HOD_LOC_CODE ");
lSBQuery.append(" where em.FORM_STATUS =1 and em.REG_STATUS in (1,2) and em.GENDER='T' and em.DCPS_OR_GPF='Y' and  ddo.ACTIVATE_FLAG = 1  and ddo.DDO_CODE <> '1111222222' and admin.LOC_ID=cadmin.LOC_ID and fld.loc_id= cfld.loc_id) as TRANS_COUNT, ");
lSBQuery.append(" (SELECT count(em.ORG_EMP_MST_ID)  FROM  mst_dcps_emp em ");
lSBQuery.append(" INNER JOIN org_ddo_mst ddo on em.DDO_CODE = ddo.DDO_CODE  ");
...
lSBQuery.append(" where em.FORM_STATUS =1 and em.REG_STATUS in (1,2) and em.DCPS_OR_GPF='Y' and  ddo.ACTIVATE_FLAG = 1  and  ddo.DDO_CODE <> '1111222222' and admin.LOC_ID=cadmin.LOC_ID and fld.loc_id= cfld.loc_id) as TOTAL_COUNT ");
...
lSBQuery.append(" INNER JOIN CMN_LOCATION_MST cfld on cfld.LOC_ID = cddo.HOD_LOC_CODE where cadmin.LOC_ID =:adminid and  cddo.ACTIVATE_FLAG = 1  group by cadmin.LOC_ID,cfld.loc_id,cfld.LOC_NAME order by cfld.loc_id  ");
...
lSBQuery.append(" INNER JOIN CMN_LOCATION_MST admin on admin.LOC_ID = ddo.DEPT_LOC_CODE ");
lSBQuery.append("  INNER JOIN CMN_LOCATION_MST fld on fld.LOC_ID = ddo.HOD_LOC_CODE  ");
lSBQuery.append(" where em.FORM_STATUS =1 and em.REG_STATUS in (1,2) and em.GENDER='M' and em.DCPS_OR_GPF = 'Y' and  ddo.ACTIVATE_FLAG = 1  and ddo.DDO_CODE <> '1111222222' and admin.LOC_ID=cadmin.LOC_ID and fld.loc_id= cfld.loc_id and ddo.DDO_CODE=cddo.DDO_CODE) as MALE_COUNT,  ");
lSBQuery.append(" (SELECT count(em.ORG_EMP_MST_ID) FROM  mst_dcps_emp em ");
lSBQuery.append(" INNER JOIN org_ddo_mst ddo on em.DDO_CODE = ddo.DDO_CODE ");
lSBQuery.append(" INNER JOIN CMN_LOCATION_MST admin on admin.LOC_ID = ddo.DEPT_LOC_CODE  ");
lSBQuery.append(" INNER JOIN CMN_LOCATION_MST fld on fld.LOC_ID = ddo.HOD_LOC_CODE ");
lSBQuery.append("  where em.FORM_STATUS =1 and em.REG_STATUS in (1,2) and em.GENDER='F' and em.DCPS_OR_GPF = 'Y' and  ddo.ACTIVATE_FLAG = 1  and ddo.DDO_CODE <> '1111222222' and admin.LOC_ID=cadmin.LOC_ID and fld.loc_id= cfld.loc_id and ddo.DDO_CODE=cddo.DDO_CODE) as FEMALE_COUNT, ");
lSBQuery.append(" (SELECT count(em.ORG_EMP_MST_ID) FROM  mst_dcps_emp em  ");
lSBQuery.append(" INNER JOIN org_ddo_mst ddo on em.DDO_CODE = ddo.DDO_CODE ");
lSBQuery.append(" INNER JOIN CMN_LOCATION_MST admin on admin.LOC_ID = ddo.DEPT_LOC_CODE	 ");
lSBQuery.append(" INNER JOIN CMN_LOCATION_MST fld on fld.LOC_ID = ddo.HOD_LOC_CODE ");
lSBQuery.append(" where em.FORM_STATUS =1 and em.REG_STATUS in (1,2) and em.GENDER='T' and em.DCPS_OR_GPF = 'Y' and  ddo.ACTIVATE_FLAG = 1  and ddo.DDO_CODE <> '1111222222' and admin.LOC_ID=cadmin.LOC_ID and fld.loc_id= cfld.loc_id and ddo.DDO_CODE=cddo.DDO_CODE) as TRANS_COUNT, ");
lSBQuery.append(" (SELECT count(em.ORG_EMP_MST_ID)  FROM  mst_dcps_emp em ");
lSBQuery.append(" INNER JOIN org_ddo_mst ddo on em.DDO_CODE = ddo.DDO_CODE  ");
lSBQuery.append("  INNER JOIN CMN_LOCATION_MST admin on admin.LOC_ID = ddo.DEPT_LOC_CODE  INNER JOIN CMN_LOCATION_MST fld on fld.LOC_ID = ddo.HOD_LOC_CODE ");
lSBQuery.append("   where em.FORM_STATUS =1 and em.REG_STATUS in (1,2) and ddo.DDO_CODE <> '1111222222' and em.DCPS_OR_GPF = 'Y' and  ddo.ACTIVATE_FLAG = 1  and admin.LOC_ID=cadmin.LOC_ID and fld.loc_id= cfld.loc_id and ddo.DDO_CODE=cddo.DDO_CODE) as TOTAL_COUNT ");
lSBQuery.append(" from mst_dcps_emp cem INNER JOIN org_ddo_mst cddo on cem.DDO_CODE = cddo.DDO_CODE ");
lSBQuery.append(" INNER JOIN CMN_LOCATION_MST cadmin on cadmin.LOC_ID = cddo.DEPT_LOC_CODE  ");
lSBQuery.append(" INNER JOIN CMN_LOCATION_MST cfld on cfld.LOC_ID = cddo.HOD_LOC_CODE where cfld.LOC_ID =:fieldid and  cddo.ACTIVATE_FLAG = 1   group by cadmin.LOC_ID,cfld.loc_id,cddo.DDO_CODE,cddo.DDO_NAMe order by cddo.DDO_CODE ");
