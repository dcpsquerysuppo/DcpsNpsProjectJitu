...
public class FormR3ReportForTreasury extends DefaultReportDataFinder implements ReportDataFinder
{
	...
	public Collection findReportData(ReportVO lObjReport, Object criteria)throws ReportException
	{
		...
		try{
			...
			String StrSqlQuery = "";
			//Added by ashish for new header 
			if (lObjReport.getReportCode().equals("8000090"))
			{
				...
				System.out.println("ddoCode  is **** "+lStrDdoCode);
				String lStrDcpsEmpId = (String) lObjReport
				.getParameterValue("dcpsEmpId");
				System.out.println("dcpsEmpId is **** "+lStrDcpsEmpId);
				...
				if ((lLstOpeningBalcTier1 != null) && (lLstOpeningBalcTier1.size() > 0))
				{
					...
					if (lObj[0] != null ||  !lObj[0].equals("") )
						lStrOpeningEmpTier1 = lObj[0].toString();

					if (lObj[1] != null || !lObj[1].equals("") )
						lStrOpeningEmployrTier1 = lObj[1].toString();

					if (lObj[2] != null || !lObj[2].equals("") )
	...
	public Double getTotalAmountDetails(Long lLngDcpsEmpId, Long lLngYearId,String fromDate,String toDate)
	{
		...
		try{
			...
			/*lSBQuery.append("SELECT OPEN_EMPLOYEE+CONTRIB_EMPLOYEE,OPEN_EMPLOYER+CONTRIB_EMPLOYER,INT_CONTRB_EMPLOYEE + INT_CONTRB_EMPLOYER ");
				...
				lQuery.setLong("dcpsEmpId", lLngDcpsEmpId);
				lQuery.setLong("year", lLngYearId);*/
			//lSBQuery.append(" SELECT sum(Empcontr),sum(TA) FROM ( ");
			//lSBQuery.append(" SELECT (CONTRIBUTION) as Empcontr , (case when EMPLOYER_CONTRI_FLAG='Y' then (contribution) else null end)  as TA FROM TRN_DCPS_CONTRIBUTION where DCPS_EMP_ID=:dcpsEmpId and REG_STATUS=1  ");			
			//lSBQuery.append(" and voucher_date between '" + fromDate+ "' AND '" + toDate + "'  ) ");
