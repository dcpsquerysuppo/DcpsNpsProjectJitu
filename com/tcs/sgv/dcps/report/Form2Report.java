	...
	throws ReportException {
		...
		try {
			...
			if (report.getReportCode().equals("700037")) {
				...
				//#A String lStrAdmBudgetCode =  lObjDcpsCommonDAO.getAdminBudgetCodeForDDO(lStrDDOCode);
				...
				System.out.println("BillGroup Id==========================="+lStrBillGroupId);
...
/*
				...
				if(Integer.parseInt(lStrMonthId) < 4){
					lSBQuery.append(" join sgvc_fin_year_mst fin on fin.fin_year_code="+(Long.valueOf(lStrYearCode)-1)+" ");
				}else if(Integer.parseInt(lStrMonthId) > 3){
					lSBQuery.append(" join sgvc_fin_year_mst fin on fin.fin_year_code="+Long.valueOf(lStrYearCode)+" ");
				}
				
				lSBQuery.append(" join MST_DCPS_BROKEN_PERIOD_PAY brk on brk.eis_emp_id=HPP.emp_id and hpp.PAYBILL_MONTH=brk.MONTH_ID and brk.year_id=fin.fin_year_id and brk.TYPE_OF_BILL=bill.TYPE_OF_BILL_GROUP "); 
				lSBQuery.append(" join RLT_DCPS_BROKEN_PERIOD_ALLOW alw on alw.RLT_BROKEN_PERIOD_ID=brk.BROKEN_PERIOD_ID "); 
				lSBQuery.append(" join HR_PAY_ALLOW_TYPE_MST alt on alt.allow_code=alw.allow_code and alt.allow_type_code='10' "); 
				lSBQuery.append(" where "); 
				lSBQuery.append(" HPP.PAYBILL_MONTH = :month ");
...
*/
			...
			/*	lSBQuery.append(" SELECT EM.dcps_id,EM.emp_name,HES.emp_id,EM.dcps_emp_id,brk.basic_pay,HPP.D_PAY,alw.allow_value,ded.DEDUC_VALUE,HPP.DCPS,EM.dcps_emp_id,HPP.CDA, ");
				lSBQuery.append(" HPP.DED_ADJUST,TO_CHAR(brk.FROM_DATE,'dd/MM/YYYY'),TO_CHAR(brk.TO_DATE,'dd/MM/YYYY'),nvl(EM.PRAN_NO,' ')   ");
				lSBQuery.append(" FROM HR_PAY_PAYBILL HPP ");
				lSBQuery.append(" join PAYBILL_HEAD_MPG PHM on HPP.PAYBILL_GRP_ID = PHM.PAYBILL_ID and PHM.APPROVE_FLAG in (0,1,5)   ");
				lSBQuery.append(" join hr_eis_emp_mst HES on HPP.EMP_ID = HES.EMP_ID   ");
				lSBQuery.append(" join MST_DCPS_EMP EM on EM.org_emp_mst_id = HES.emp_mpg_id   ");
				...
				lSBQuery.append(" join RLT_DCPS_BROKEN_PERIOD_ALLOW alw on alw.RLT_BROKEN_PERIOD_ID=brk.BROKEN_PERIOD_ID   ");
				lSBQuery.append(" join HR_PAY_ALLOW_TYPE_MST alt  on alt.allow_code=alw.allow_code and alt.allow_type_code='10' ");
				lSBQuery.append(" join RLT_DCPS_BROKEN_PERIOD_DEDUC ded on ded.RLT_BROKEN_PERIOD_ID=brk.BROKEN_PERIOD_ID and ded.DEDUC_CODE=59 ");
				lSBQuery.append(" where ");
				lSBQuery.append(" HPP.PAYBILL_MONTH = :month ");
				lSBQuery.append(" and HPP.PAYBILL_YEAR = :year ");
				lSBQuery.append(" and PHM.BILL_NO = :billGroupId ");
...
*/
				...
				lSBQuery.append(" SELECT EM.dcps_id,EM.emp_name,HES.emp_id,EM.dcps_emp_id, case when tmp.basic_pay is null then hpp.PO else tmp.basic_pay end, HPP.D_PAY, case when tmp.allow_value is null then hpp.DA else tmp.allow_value end, case when tmp.DEDUC_VALUE is null then hpp.dcps else tmp.DEDUC_VALUE end,EM.dcps_emp_id,HPP.CDA,HPP.DED_ADJUST,TO_CHAR(tmp.FROM_DATE,'dd/MM/YYYY'),TO_CHAR(tmp.TO_DATE,'dd/MM/YYYY'),nvl(EM.PRAN_NO,' ') ");//HPP.DCPS,
				lSBQuery.append(" FROM HR_PAY_PAYBILL HPP  ");
				lSBQuery.append(" join PAYBILL_HEAD_MPG PHM on HPP.PAYBILL_GRP_ID = PHM.PAYBILL_ID and PHM.APPROVE_FLAG in (0,1,5)    ");
				lSBQuery.append(" join hr_eis_emp_mst HES on HPP.EMP_ID = HES.EMP_ID    ");
				lSBQuery.append(" join MST_DCPS_EMP EM on EM.org_emp_mst_id = HES.emp_mpg_id    ");
				lSBQuery.append(" join CMN_LOOKUP_MST look on em.AC_DCPS_MAINTAINED_BY = look.LOOKUP_ID    ");
				lSBQuery.append(" join MST_DCPS_BILL_GROUP bill on phm.BILL_NO=bill.BILL_GROUP_ID    ");
				lSBQuery.append(" LEFT OUTER JOIN (SELECT brk.FROM_DATE,brk.to_Date,alw.allow_value,ded.DEDUC_VALUE,brk.basic_pay,brk.eis_emp_id,brk.MONTH_ID,brk.year_id,brk.TYPE_OF_BILL FROM MST_DCPS_BROKEN_PERIOD_PAY brk   ");  
				...
				lSBQuery.append("  ORDER BY BRK.BROKEN_PERIOD_ID ) AS TMP ON  ");
				lSBQuery.append(" TMP.eis_emp_id=HPP.emp_id  and TMP.TYPE_OF_BILL=bill.TYPE_OF_BILL_GROUP ");
				lSBQuery.append(" where  ");
				lSBQuery.append(" HPP.PAYBILL_MONTH = :month ");
				lSBQuery.append(" and HPP.PAYBILL_YEAR = :year ");
				lSBQuery.append(" and PHM.BILL_NO = :billGroupId ");
				...
				gLogger.info(" month ####### :   "+lStrMonthId);
				List lLstFinal = Query.list();
...
*/				List lListEmpListBrokenPeriodDCPS = lObjOfflineContriDAO.getEmpListOfDCPSBrokenPeriodForMonth(lLongYearId,lLongMonthId);
			...
			//		if(!BrokenPeriodFlagForEmployee)
				//	{
					...
					//
						...
						else
						{
							...
							if (tuple[11] != null && tuple[12] != null)
							{
								td8.add(tuple[11].toString()+" - "+tuple[12].toString());
							}else 
							{
								td8.add("");
							}
				...
				if(!lStrAccMaintainedBy.equals(""))
				{
					...
					if(lStrAccMaintainedBy.equals("700240"))
					{
						...
						.append("Head of Account 8342-Other Deposits 00,117-Government Employees Defined Contribution Pension scheme - (02)(08)Defined Contribution Pension scheme - Employee Contribution(IAS) Tier-1 (8342 536-601) ");
					...
					else if(lStrAccMaintainedBy.equals("700241"))
					{
						...
						"Add - Employer's contribution");
						lStrThirdLastLineInTable
						.append("Head of Account 8342-Other Deposits 00,117-Government Employees Defined Contribution Pension scheme - (02)(09)Defined Contribution Pension scheme - Employee Contribution(IPS) Tier-1 (8342 502-601) ");
			...
			/*	if((lStrDDOCode.equals("1111222222") || lStrDDOCode.equals("7101003272")) && totalDed!=0){*/
				...
				rowList.add(new StyledData("Total Amount Ruppes: "
						+ totalAmt+ "/-", rowsFontsVO));
				dataList.add(rowList);

				 lStrTotAmtInWords = EnglishDecimalFormat
				...
				rowList
				.add(new StyledData(
						"1: Certified that the name amount of the individual's deduction and the total shown in column(8) have been checked with reference to the bill,vide,paragraph 224 of the Audit Manual",
						rowsFontsVO));
				dataList.add(rowList);
	...
	public List getBillGroup(Hashtable otherArgs, String lStrLangId,
		...
		try {
			...
			sb.append("SELECT dcpsDdoBillGroupId, dcpsDdoBillDescription FROM MstDcpsBillGroup WHERE billDeleted is null and dcpsDdoCode=7101065774");
			if(lStrDdoCode!=null && !lStrDdoCode.equals("") && lStrDdoCode.length()==10){
				sb.append(" and dcpsDdoCode = :dcpsDdoCode ");
			}
			sb.append(" ORDER BY dcpsDdoBillDescription");

			Query lObjQuery = lObjSession.createQuery(sb.toString());
			if(lStrDdoCode!=null && !lStrDdoCode.equals("") && lStrDdoCode.length()==10){
				lObjQuery.setString("dcpsDdoCode", lStrDdoCode);
			}

			gLogger.info("Query for report is"+sb.toString());
			gLogger.info("DDO code is is"+lStrDdoCode);
			//lObjQuery.setString("dcpsDdoCode", lStrDdoCode);
